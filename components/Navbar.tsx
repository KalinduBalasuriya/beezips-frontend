...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-cream/50 backdrop-blur border-b border-honey/20">
      <nav className="mx-auto max-w-7xl px-4 lg:px-10 flex items-center justify-between h-12 sm:h-14">
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="font-display text-lg sm:text-xl font-700 text-ink tracking-tight">
            Beezips
          </span>
        </Link>
        ...
        <button
          ...
          aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
          className="md:hidden text-cream w-7 h-7 flex flex-col justify-center gap-1"
        >
          <span
      ...
      {open && (
        <div className="md:hidden bg-ink/10 backdrop:blur px-6 py-4 flex flex-col gap-2">
          {links.map((l) => (
            <Link
              key={l.label}
              href={l.href}
              onClick={() => setOpen(false)}
              className="text-ink/90 hover:text-honey text-sm font-600  tracking-wide"
            >
              {l.label}
            </Link>
          ))}
